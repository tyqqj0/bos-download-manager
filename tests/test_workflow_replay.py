...
def test_history_replays_against_current_workflow_code(filename):
    ...
    replayer = Replayer(workflows=[
        ...
        DownloadDatasetWorkflow,
        SplitDownloadWorkflow,
        # Every workflow type a recorded history can name has to be registered
        # here, or the replay fails on "not registered on this worker" — which
        # looks identical to a determinism break but is a harness gap.
        PoolDownloadWorkflow,
    ])
