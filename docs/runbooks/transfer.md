...
一个例外要知道：远端任务列表只返回最新 500 条（实测 2026-08-10：确实是按创建时间
倒序，500 条能回溯到 2026-06-28，约六周），所以**跑得久的 import 理论上会从列表里
掉出去**——需要它在飞的期间那边新建了 500 个任务，不常见但不是不可能。这时我们不猜，
改去量目的端：量够了就 `done`，不够就**留在
